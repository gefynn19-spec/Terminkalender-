...
const namen = { 'Main.dc.html': 'endkarte' };
...
for (const [file, name] of Object.entries(namen)) {
  ...
  if (Math.round(box.height) !== 1920) console.log('!! ' + name + ' ist ' + Math.round(box.height) + 'px hoch');
  await el.screenshot({ path: path.join(OUT, `tiktok-${name}.png`) });
  n++;
}
